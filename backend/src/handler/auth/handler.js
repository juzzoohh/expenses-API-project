...
const loginUserHandler = async (request, h) => {
  ...
  if (!username || !password) {
    const response = h.response({
      status: 'fail',
      // Jangan bilang "Username tidak ditemukan".
      // Bilang "Username atau Password salah" untuk membingungkan hacker mana yang salah.
  ...
  // result.rows.length === 0 artinya tidak ada baris yang cocok 
  ...
  // buatkan user "Kartu Akses Elektronik" (JWT).
